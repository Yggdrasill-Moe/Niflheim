...
unit32 process_dir(char* dname)
{
	...
	while (q->next)
	{
		...
		do
		{
			...
			if (row->NameLength % 4 == 0)//四字节对齐
				row->IndexLength = 0x18 + row->NameLength;
			else
				row->IndexLength = 0x18 + row->NameLength + (4 - (row->NameLength % 4));
...
void CPZHeaderEncrypt()
{
	...
	CPZ_Header.HeaderCRC = CheckCRC((unit8*)&CPZ_Header, 0x3C, InitCheckcrc);
}
...
void MakeRandData()
{
	...
	cmvsMd5Data[0] ^= 0x45A76C2F;
	cmvsMd5Data[1] -= 0x5BA17FCB;
	cmvsMd5Data[2] ^= 0x79ABE8AD;
	cmvsMd5Data[3] -= 0x1C08561B;
}

void MakeCPZ6File(char* fname)
{
	...
	if (CPZ_Header.DirCount > 0)
	{
		...
		fseek(dst, sizeof(CPZ_Header) + CPZ_Header.DirIndexLength + CPZ_Header.FileIndexLength, SEEK_SET);
		_chdir(fname);
		q = CPZ_Dir_Index;
		while (q->next)
		{
			...
			while (p)
			{
				...
				if (CPZ_Header.IsEncrypt)
					CPZResourceEncrypt(data, p->Length, CPZ_Header.IndexKey, cmvsMd5Data, IndexSeed ^ ((CPZ_Header.IndexKey ^ (q->DirKey + p->FileKey)) + CPZ_Header.DirCount + 0xa3d61785));
				p->CRC = CheckCRC(data, p->Length, 0x5A902B7C);//sub_455D90 in ChronoClock
				p->Offset = ftell(dst) - sizeof(CPZ_Header) - CPZ_Header.DirIndexLength - CPZ_Header.FileIndexLength;
		...
		_chdir("..");
		//构造index
		data = malloc(CPZ_Header.DirIndexLength + CPZ_Header.FileIndexLength);
		...
		while (q->next)
		{
			...
			while (p)
			{
				...
				fwrite(p->FileNameSave, p->IndexLength - 0x18, 1, dst);
				p = p->next;
			}
		}
		fseek(dst, sizeof(CPZ_Header), SEEK_SET);
		...
		IndexMD5(data, CPZ_Header.DirIndexLength + CPZ_Header.FileIndexLength);
		fseek(dst, sizeof(CPZ_Header), SEEK_SET);
		fwrite(data, CPZ_Header.DirIndexLength + CPZ_Header.FileIndexLength, 1, dst);
		free(data);
		//写CPZ_Header
		CPZ_Header.Magic = 0x365A5043;
		CPZHeaderEncrypt();
