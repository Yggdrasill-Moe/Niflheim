...
void PackFile(char *fname)
{
	...
	_fseeki64(dst, GAR_Header2.first_offset, SEEK_SET);
	for (i = 0; i < GAR_Header2.num - 1; i++)
	{
		if (Index[i].number == GAR_Header.num_start)
		{
			if (i == GAR_Header2.num - 2)
				continue;
			else
			{
				printf("编号%d不是文件的最后一个块，可能此文件是无法处理的格式！\n", i);
				system("pause");
			}
		}
		if (Index[i].number > GAR_Header.num_start)
		...
		Index[i].offset = _ftelli64(dst);
		fp = fopen(Index[i].filename, "rb");
		if (fp == NULL)
		{
			printf("%s文件不存在！\n", Index[i].filename);
			system("pause");
		...
		Index[i].size = _ftelli64(fp);
		_fseeki64(fp, 0, SEEK_SET);
		data = malloc((unit32)Index[i].size);//TMD一个GAL不会有大于2G的视频吧.........
		fread(data, (unit32)Index[i].size, 1, fp);
	...
	Index[GAR_Header2.num - 2].offset = _ftelli64(dst);
	Index[GAR_Header2.num - 2].size = 0x14 * GAR_Header2.num + 0x20;
	GAR_Header.index_off = _ftelli64(dst);
	GAR_Header2.size = Index[GAR_Header2.num - 2].size + Index[GAR_Header2.num - 2].offset;
	...
	_fseeki64(dst, 0, SEEK_SET);
	fwrite(&GAR_Header, sizeof(GAR_Header), 1, dst);
	_fseeki64(dst, GAR_Header.index_off, SEEK_SET);
	fwrite(&GAR_Header2, sizeof(GAR_Header2), 1, dst);
	for (i = 0; i < GAR_Header2.num - 1; i++)
		fwrite(&Index[i], 0x14, 1, dst);
	fclose(src);
	fclose(dst);
}
