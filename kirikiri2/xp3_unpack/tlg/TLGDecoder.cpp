...
BOOL  InitBitmapHeader(IMG_BITMAP_HEADER  *pHeader, Int32 Width, Int32 Height, Int32 BitsPerPixel, PInt32 pStride)
{    
    Int32 Stride;
    ...
    pHeader->wTag = (UInt16)0x424D; //TAG2('BM');
...
//    CMem mem;
    ...
    pbOutBuffer = (PBYTE)malloc(stride * height + sizeof(BitmpHeader));
    if (pbOutBuffer == NULL)
        return FALSE;
    ...
    *(SBitMapHeader *)pbOutBuffer = BitmpHeader;
    ...
    inbuf = (UInt8*)malloc(blockheight * width + 10);
    for(Int i = 0; i < colors; i++)
        outbuf[i] = (UInt8*)malloc(blockheight * width + 10);
